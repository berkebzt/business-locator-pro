...
class BusinessLocatorPro {
  ...
  initializeMarkerCluster() {
    ...
    this.markerClusterGroup = L.markerClusterGroup({
      maxClusterRadius: 80,           // Group radius in pixels
      disableClusteringAtZoom: 17,    // Uncluster at high zoom levels
      spiderfyOnMaxZoom: true,        // Spiderfy when zoomed in
    ...
    // Add cluster click event for better UX
    this.markerClusterGroup.on('clusterclick', (e) => {
      console.log(`🔍 Cluster clicked with ${e.layer.getChildCount()} markers`);
    });
    
    // Handle spiderfy events
    this.markerClusterGroup.on('spiderfied', (e) => {
      console.log(`🕷️ Cluster spiderfied with ${e.markers.length} markers`);
    });
    
    console.log('🗺️ Marker clustering initialized');
  }
  ...
  displayMarkers(data) {
    ...
    console.log(`🗺️ Displaying ${data.length} businesses with clustering`);
    ...
    data.forEach(business => {
      ...
      popupText += `
          <div class="popup-actions">
            ...
            <button onclick="window.businessLocator.toggleFavorite('${business.name}', ${business.lat}, ${business.lng}, '${business.type}')" class="popup-btn favorite-btn" id="favorite-${business.name.replace(/\s+/g, '-')}">
              <i class="fas fa-heart"></i> <span class="favorite-text">Add to Favorites</span>
            </button>
      ...
      // Add marker to cluster group instead of regular marker group
    ...
    // Update favorite buttons after a short delay to ensure popups are rendered
    setTimeout(() => {
  ...
  loadFavorites() {
    try {
      const saved = localStorage.getItem('businessLocatorFavorites');
  ...
  saveFavorites() {
    try {
      localStorage.setItem('businessLocatorFavorites', JSON.stringify(this.favorites));
      this.updateFavoritesCount();
  ...
  isFavorite(businessName) {
    return this.favorites.some(fav => fav.name === businessName);
  }

  addFavorite(business) {
    if (!this.isFavorite(business.name)) {
      this.favorites.push({
        name: business.name,
        lat: business.lat,
        lng: business.lng,
      ...
      this.saveFavorites();
      this.updateFavoriteButton(business.name, true);
      this.showStatus(`Added "${business.name}" to favorites!`, 'success');
    }
  }

  removeFavorite(businessName) {
    this.favorites = this.favorites.filter(fav => fav.name !== businessName);
    this.saveFavorites();
    this.updateFavoriteButton(businessName, false);
    this.showStatus(`Removed "${businessName}" from favorites`, 'info');
  }

  toggleFavorite(name, lat, lng, type) {
    const business = { name, lat, lng, type };
  ...
  updateFavoriteButton(businessName, isFavorite) {
    const buttonId = `favorite-${businessName.replace(/\s+/g, '-')}`;
  ...
  updateAllFavoriteButtons() {
    this.businesses.forEach(business => {
      this.updateFavoriteButton(business.name, this.isFavorite(business.name));
    });
  }
  ...
  displayFavorites() {
    ...
    // Sort favorites by distance if user location is available
    ...
    sortedFavorites.forEach(favorite => {
      const distance = this.userLocation ? 
        this.calculateDistance(this.userLocation.lat, this.userLocation.lng, favorite.lat, favorite.lng) : 
        'N/A';
      
      const favoriteItem = document.createElement('div');
      favoriteItem.className = 'favorite-item';
      favoriteItem.innerHTML = `
        <div class="favorite-icon" style="background-color: ${this.getIconColor(favorite.type)}">
          ${this.getIconEmoji(favorite.type)}
        </div>
        <div class="favorite-info">
          <div class="favorite-name">${favorite.name}</div>
          <div class="favorite-details">
            <span class="favorite-type">${favorite.type}</span>
            <span class="favorite-distance">${distance} km</span>
          </div>
        </div>
        <div class="favorite-actions">
          <button class="favorite-action-btn view-btn" onclick="window.businessLocator.centerOnBusiness('${favorite.name}')" title="View on Map">
            <i class="fas fa-map-marker-alt"></i>
          </button>
          <button class="favorite-action-btn remove-btn" onclick="window.businessLocator.removeFavoriteFromModal('${favorite.name}')" title="Remove">
  ...
  removeFavoriteFromModal(businessName) {
    this.removeFavorite(businessName);
    this.displayFavorites(); // Refresh the modal
  }

  exportFavorites() {
    if (this.favorites.length === 0) {
      this.showStatus('No favorites to export', 'error');
      return;
    }
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `business_favorites_${new Date().toISOString().split('T')[0]}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    window.URL.revokeObjectURL(url);
    
    this.showStatus('Favorites exported successfully!', 'success');
  }

  clearAllFavorites() {
    if (this.favorites.length === 0) {
      this.showStatus('No favorites to clear', 'info');
      return;
    }
    
    if (confirm(`Are you sure you want to remove all ${this.favorites.length} favorites?`)) {
      this.favorites = [];
      this.saveFavorites();
      this.displayFavorites();
      this.showStatus('All favorites cleared', 'info');
  ...
  getClusterStats() {
    if (!this.markerClusterGroup) return null;
    
    const stats = {
      totalMarkers: this.markerClusterGroup.getLayers().length,
      clusters: this.markerClusterGroup.getLayers().filter(layer => layer.getChildCount).length,
      individualMarkers: this.markerClusterGroup.getLayers().filter(layer => !layer.getChildCount).length
    };
